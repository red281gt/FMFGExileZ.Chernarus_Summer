...
_null = [Player] execVM "FMFG\IgiLoad\IgiLoad.sqf";
...

{
	if ((typeOf _x) in (IL_Supported_Vehicles_All)) then
	{
		...
		_null = [_x] execVM "FMFG\IgiLoad\IgiLoad.sqf";
		waitUntil {scriptDone _null};
	};
} forEach (vehicles);

call compileFinal preprocessFileLineNumbers "FMFG\IgiLoad\IgiLoadTaru.sqf";
