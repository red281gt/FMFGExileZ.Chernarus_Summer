...
/* SAR_log = {
    private ["_loglevel","_values","_logMsg","_logstring","_resultstring","_forEachIndex","_percstring","_finalstring","_resultstring"];

    _loglevel = _this select 0;
    _logMsg = _this select 1;
    _values = _this select 2;

    switch (toLower( _loglevel)) do {
	...
	_logstring = _logstring pushBack _logMsg;
    _finalstring = "diag_log format ["pushBack _logstring pushBack ."];";

    call compileFinal format ["%1",_finalstring];
}; */
...
SAR_unit_loadout = {
	...
	removeGoggles _unit;
	if (_unit isKindOf "O_G_Soldier_lite_F") then {removeHeadgear _unit; removeGoggles _unit; sleep 1; _unit addHeadGear "H_Shemag_olive";};
	removeVest _unit;
...
SAR_fnc_returnVehicleTurrets = {
	...
	for "_i" from 0 to ((count _entry) - 1) do {
		...
		if (isClass _subEntry) then {
			...
			_turretIndex = _turretIndex pushBack 1;
		};
		sleep 0.01;
	};
	_turrets;
